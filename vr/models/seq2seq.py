...
class Seq2Seq(nn.Module):
    def __init__(self,
        ...
        self.decoder_rnn = nn.LSTM(wordvec_dim + hidden_dim, hidden_dim,
    ...
    def before_rnn(self, x, replace=0):
        ...
        for i in range(N):
            for t in range(T - 1):
                if (x_cpu.data[i, t] != self.NULL and
                   x_cpu.data[i, t + 1] == self.NULL):
                    idx[i] = t
                    break
    ...
    def decoder(self, encoded, y, h0=None, c0=None):
        ...
        output_logprobs = self.decoder_linear(rnn_output_2d).view(
            N, T_out, V_out)
    ...
    def compute_loss(self, output_logprobs, y):
        """
        Compute loss. We assume that the first element of the output sequence y
        is a start token, and that each element of y is left-aligned and
        right-padded with self.NULL out to T_out. We want the output_logprobs
        to predict the sequence y, shifted by one timestep so that y[0] is fed
        to the network and then y[1] is predicted. We also don't want to
        compute loss for padded timesteps.
    ...
    def reinforce_sample(self, x, max_length=30, temperature=1.0,
                         argmax=False):
    ...
    def reinforce_backward(self, reward, output_mask=None):
        """
        If output_mask is not None, then it should be a FloatTensor of
        shape (N, T) giving a multiplier to the output.
        """
