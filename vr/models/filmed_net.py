...
class FiLMedNet(nn.Module):
    def __init__(self, vocab, feature_dim=(1024, 14, 14),
        ...
        self.num_cond_maps = (2 * self.module_dim
                              if self.condition_method == 'concat' else 0)
        self.fwd_count = 0
        ...
        self.default_weight = Variable(
            torch.ones(1, 1, self.module_dim)).type(torch.cuda.FloatTensor)
        self.default_bias = Variable(
            torch.zeros(1, 1, self.module_dim)).type(torch.cuda.FloatTensor)

        # Initialize stem
        stem_feature_dim = feature_dim[0] + (self.stem_use_coords *
    ...
    def forward(self, x, film, save_activations=False):
        ...
        if self.condition_method == 'concat':
            """ Use parameters usually used to condition via FiLM instead to
            ...
            gammas, betas = torch.split(film[:, :, :2*self.module_dim],
        ...
        module_inputs = Variable(torch.zeros(feats.size()).unsqueeze(1).expand(
            N, self.num_modules, self.module_dim, H, W)).type(
        ...
        for fn_num in range(self.num_modules):
            if self.condition_method == 'concat':
                ...
                layer_output = self.function_modules[fn_num](
                    module_inputs[:, fn_num], gammas[:, fn_num, :],
                    betas[:, fn_num, :], batch_coords)
            ...
            if fn_num == (self.num_modules - 1):
                ...
                module_inputs_updated[:, fn_num+1] = module_inputs_updated[
                    :, fn_num+1] + layer_output
        ...
        if (((self.fwd_count % self.debug_every) == 0) or
           (self.debug_every <= -1)):
            pdb.set_trace()
        return out
...
class FiLMedResBlock(nn.Module):
    def __init__(self, in_dim, out_dim=None, with_residual=True,
                 with_batchnorm=True,
                 with_cond=[False], dropout=0, num_extra_channels=0,
                 extra_channel_freq=1,
                 with_input_proj=0, num_cond_maps=0, kernel_size=3,
                 batchnorm_affine=False,
                 num_layers=1, condition_method='bn-film',
                 debug_every=float('inf')):
        if out_dim is None:
            out_dim = in_dim
        ...
        self.extra_channel_freq = (
            0 if num_extra_channels == 0 else extra_channel_freq)
        # Kernel size of input projection
        self.with_input_proj = with_input_proj
        ...
        if self.with_input_proj:
            self.input_proj = nn.Conv2d(in_dim + (
                num_extra_channels
                if self.extra_channel_freq >= 1 else 0),
                in_dim, kernel_size=self.with_input_proj,
                padding=self.with_input_proj // 2)

        self.conv1 = nn.Conv2d(
            in_dim + self.num_cond_maps +
        ...
        if self.with_batchnorm:
            self.bn1 = nn.BatchNorm2d(
                out_dim,
                affine=((not self.with_cond[0]) or self.batchnorm_affine))
    ...
    def forward(self, x, gammas=None, betas=None, extra_channels=None,
                cond_maps=None):
        if self.debug_every <= -2:
            pdb.set_trace()
...
def coord_map(shape, start=-1, end=1):
    ...
    """
    m, n = shape
    x_coord_row = torch.linspace(start, end,
                                 steps=n).type(torch.cuda.FloatTensor)
    y_coord_row = torch.linspace(start, end,
                                 steps=m).type(torch.cuda.FloatTensor)
