...
def build_classifier(module_C, module_H, module_W, num_answers,
    ...
    if 'maxpool' in downsample or 'avgpool' in downsample:
        ...
        # Potentially sub-optimal padding for non-perfectly aligned pooling
        padding = (0 if ((module_H % pool_size == 0) and
                   (module_W % pool_size == 0)) else 1)
        layers.append(pool(kernel_size=pool_size, stride=pool_size,
                           padding=padding))
        prev_dim = (proj_dim * math.ceil(module_H / pool_size)
                    * math.ceil(module_W / pool_size))
    if downsample == 'aggressive':
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        layers.append(nn.AvgPool2d(kernel_size=module_H // 2,
                                   stride=module_W // 2))
        prev_dim = proj_dim
