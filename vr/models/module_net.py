...
class ModuleNet(nn.Module):
    ...
    def expand_answer_vocab(self, answer_to_idx, std=0.01, init_b=-50):
        """ TODO: This is really gross, dipping into private internals of
        Sequential"""
    ...
    def _forward_modules_json(self, feats, program):
        ...
        for i in range(N):
            ...
            for j, f in enumerate(program[i]):
                ...
                if self.save_module_outputs:
                    self.all_module_outputs[-1].append(
                        module_outputs[-1].data.cpu().clone())
    ...
    def _forward_modules_ints_helper(self, feats, program, i, j):
        ...
        if fn_str == 'scene':
            ...
            while len(module_inputs) < num_inputs:
                cur_input, j = self._forward_modules_ints_helper(
                    feats, program, i, j)
    ...
    def _forward_modules_ints(self, feats, program):
        ...
        for i in range(N):
            cur_output, _ = self._forward_modules_ints_helper(
                feats, program, i, 0)
