...
class FiLMGen(nn.Module):
    def __init__(self,
        ...
        # FiLM params per ResBlock
        self.cond_feat_size = 2 * self.module_dim * self.module_num_layers
        # parameter_efficient=False only used to load older trained models
        if not parameter_efficient:
            self.cond_feat_size = 4 * self.module_dim + 2 * self.num_modules

        self.encoder_embed = nn.Embedding(encoder_vocab_size, wordvec_dim)
        self.encoder_rnn = init_rnn(self.encoder_type, wordvec_dim, hidden_dim,
    ...
    def encoder(self, x):
        ...
        # Tokenized word sequences (questions), end index
    ...
    def modify_output(self, out, gamma_option='linear', gamma_scale=1,
                      gamma_shift=0, beta_option='linear', beta_scale=1,
                      beta_shift=0):
        ...
        for i in range(self.module_num_layers):
            gs.append(slice(i * (2 * self.module_dim),
                            i * (2 * self.module_dim) + self.module_dim))
            bs.append(slice(i * (2 * self.module_dim) + self.module_dim,
                            (i + 1) * (2 * self.module_dim)))
...
def init_rnn(rnn_type, hidden_dim1, hidden_dim2, rnn_num_layers,
    ...
    if rnn_type == 'gru':
        return nn.GRU(hidden_dim1, hidden_dim2, rnn_num_layers,
                      dropout=dropout, batch_first=True,
                      bidirectional=bidirectional)
    elif rnn_type == 'lstm':
        return nn.LSTM(hidden_dim1, hidden_dim2, rnn_num_layers,
                       dropout=dropout, batch_first=True,
                       bidirectional=bidirectional)
