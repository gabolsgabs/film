...
# -1 for all shuffled
parser.add_argument('--num_last_words_shuffled', default=0, type=int)
...
grads = {}
# NOTE: Useful for zero-shot program manipulation when in debug mode
programs = {}
...
def main(args):
    ...
    elif (args.program_generator is not None and
          args.execution_engine is not None):
    ...
    # Interactive mode
    elif (args.image is not None and args.input_question_h5 is None and
          args.input_features_h5 is None):
...
def extract_image_features(args, dtype):
    ...
    img_var = Variable(torch.FloatTensor(img).type(dtype), volatile=False,
                       requires_grad=True)
...
def run_single_example(args, model, dtype, question_raw, feats_var=None):
    ...
    vocab = load_vocab(args)
    question_tokens = tokenize(question_raw, punct_to_keep=[';', ','],
                               punct_to_remove=['?', '.'])
    ...
    if type(model) is tuple:
        ...
        if args.model_type == 'FiLM':
            ...
            predicted_program = pg.reinforce_sample(
                question_var, temperature=args.temperature,
    ...
    for i in range(len(answers_to_probs_sorted)):
        if (answers_to_probs_sorted[i][1] >= 1e-3 and
           args.debug_every < float('inf')):
...
def run_our_model_batch(args, pg, ee, loader, dtype):
    ...
    for batch in tqdm(loader):
        ...
        if args.num_last_words_shuffled != 0:
            for i, question in enumerate(questions):
                ...
                q_end = get_index(question.numpy().tolist(), index=2,
        ...
        if isinstance(questions, list):
            questions_var = Variable(questions[0].type(dtype).long(),
                                     volatile=True)
        ...
        if args.model_type == 'FiLM':
            ...
            programs_pred = pg.modify_output(
                programs_pred, gamma_option=args.gamma_option,
            ...
            programs_pred = pg.reinforce_sample(
                questions_var, temperature=args.temperature,
                argmax=(args.sample_argmax == 1))
    ...
    if args.output_h5 is not None:
        ...
        with h5py.File(args.output_h5, 'w') as fout:
            ...
            fout.create_dataset('predicted_programs',
                                data=all_programs.numpy())
    ...
    if args.output_program_stats_dir:
        ...
        torch.save(gamma_means, os.path.join(args.output_program_stats_dir,
                                             'gamma_means'))
        beta_means = betas.mean(0)
        torch.save(beta_means, os.path.join(args.output_program_stats_dir,
                                            'beta_means'))
        gamma_medians = gammas.median(0)[0]
        torch.save(gamma_medians, os.path.join(args.output_program_stats_dir,
                                               'gamma_medians'))
        beta_medians = betas.median(0)[0]
        torch.save(beta_medians, os.path.join(args.output_program_stats_dir,
                                              'beta_medians'))

        # Note: Takes O(10GB) space
        torch.save(gammas, os.path.join(args.output_program_stats_dir,
                                        'gammas'))
        torch.save(betas, os.path.join(args.output_program_stats_dir,
                                       'betas'))
    ...
    if args.output_preds is not None:
        ...
        for i in range(len(all_preds)):
            all_preds_strings.append(
                vocab['answer_idx_to_token'][all_preds[i]])
...
def visualize(features, args, file_name=None):
        ...
        Then saves the input RGB image as an RGBA image using an upsampling of
        this attention map.
        """
        ...
        f_map_scaled = f_map_shifted / f_map_shifted.max().expand_as(
            f_map_shifted)
        ...
        if save_file is None:
            ...
            alpha_upsampled = torch.nn.functional.upsample_bilinear(
                alpha4d, size=torch.Size(orig_img_size)).squeeze(0).transpose(
                    1, 0).transpose(1, 2)
