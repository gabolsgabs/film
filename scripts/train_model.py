...
parser.add_argument(
    '--condition_method', default='bn-film', type=str,
    choices=['block-input-film', 'block-output-film', 'bn-film', 'concat',
             'conv-film', 'relu-film'])
...
parser.add_argument(
    '--classifier_downsample', default='maxpool2',
    choices=['maxpool2', 'maxpool3', 'maxpool4', 'maxpool5', 'maxpool7',
             'maxpoolfull', 'none', 'avgpool2', 'avgpool3', 'avgpool4',
             'avgpool5', 'avgpool7', 'avgpoolfull', 'aggressive'])
...
parser.add_argument('--optimizer', default='Adam',
                    choices=['Adadelta', 'Adagrad', 'Adam', 'Adamax', 'ASGD',
                             'RMSprop', 'SGD'])
...
def get_execution_engine(args):
    ...
    if args.execution_engine_start_from is not None:
        ...
        if args.model_type == 'FiLM':
            ...
            kwargs['module_num_layers'] = args.module_num_layers
            kwargs['module_batchnorm_affine'] = (
                args.module_batchnorm_affine == 1)
    ...
    for batch in loader:
        ...
        if args.model_type == 'PG':
            ...
            for i in range(questions.size(0)):
                program_pred = program_generator.sample(Variable(
                    questions[i:i+1].cuda(), volatile=True))
        ...
        if (args.num_val_samples is not None and
...
def check_grad_num_nans(model, model_name='model'):
        ...
        num_nans = [np.sum(np.isnan(grad.data.cpu().numpy()))
                    for grad in grads]
